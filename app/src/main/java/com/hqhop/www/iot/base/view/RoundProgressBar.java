...
public class RoundProgressBar extends View {
    ...
    public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        textColor = mTypedArray.getColor(R.styleable.RoundProgressBar_textColor2, Color.GREEN);
        textSize = mTypedArray.getDimension(R.styleable.RoundProgressBar_textSize, 15);
        roundWidth = mTypedArray.getDimension(R.styleable.RoundProgressBar_roundWidth, 5);
        max = mTypedArray.getInteger(R.styleable.RoundProgressBar_max, 100);
        textIsDisplayable = mTypedArray.getBoolean(R.styleable.RoundProgressBar_textIsDisplayable, true);
        style = mTypedArray.getInt(R.styleable.RoundProgressBar_style, 0);
        type = mTypedArray.getString(R.styleable.RoundProgressBar_textType);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float typeWidth = paint.measureText(type);   //测量字体宽度，我们需要根据字体的宽度设置在圆环中间

//        if (textIsDisplayable && percent != 0 && style == STROKE) {
        if (textIsDisplayable && style == STROKE) {
//            canvas.drawText(percent + "%", centre - textWidth / 2, centre + textSize / 2, paint); //画出进度百分比
            canvas.drawText(String.valueOf(percent), centre - textWidth / 2, centre - textSize / 2, paint); //画出进度百分比
            canvas.drawText(type, centre - typeWidth / 2, centre + typeWidth / 2, paint);// 画出类别文字
        }
